...
> **实现状态（M3 ✅ 2026-07-12）**：保底轨（升级 +1→+9）+ 赌博轨（+3/+6/+9 抽词条 70%正面/30%诅咒，+9保底正面）+ 经济循环（gold_per_sec/per_kill/gold_mult/double_gold）已全部实现。掉落轨首版不做。诅咒 cost+benefit 合并生效，战斗效果走 EffectResolver，经济效果走 getter。27 词条（19 正面 + 8 诅咒）数据完整。
...
- ~~`bond_owned`~~：**已取消**。原设计"拥有单个羁绊就触发"门槛太低，一个羁绊+一个词条就能触发联动，不合理。
- `bond_devoured_set`：该体系**已修满顶级境界**（吞噬完成）。**所有联动统一走此条件**——联动是修炼到顶的终极奖励，不是随便凑。
- 即：所有 9 条联动都需要"吞噬某体系 + 另一系统条件"才能触发。圣体真伤改为"吞噬遮天 + 真伤词条"。联动 = 终极 combo，门槛高、回报大。
- > **设计修订理由**：(1) 原设计吞噬后失去联动，惩罚修炼 → 改为吞噬=获得；(2) bond_owned 门槛太低 → 取消，统一走 bond_devoured_set。修炼路径 = 纯正反馈，修满才有联动奖励。
